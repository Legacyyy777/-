...
if [ -f "backend/.env" ]; then
    ...
    # Проверяем что пароль не содержит $ (значит это переменная, а не реальный пароль)
    if [[ "$DB_PASSWORD" == *'$'* ]] || [ -z "$DB_PASSWORD" ]; then
        DB_PASSWORD=""
    fi
fi

# Если пароль пустой или неправильный - генерируем новый
if [ -z "$DB_PASSWORD" ]; then
    DB_PASSWORD=$(openssl rand -base64 24 | tr -d "=+/" | cut -c1-20)
    echo -e "${YELLOW}   Сгенерирован новый пароль${NC}"
fi
